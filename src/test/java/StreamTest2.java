...
public class StreamTest2 {
    ...
    void flatMap_test() {
        ...
        List<String> flatMapList = Arrays.stream(cities).flatMap(Arrays::stream).collect(Collectors.toList());

        System.out.println(flatMapList);

        Assertions.assertEquals(flatMapList.size(), Arrays.stream(cities).flatMap(c -> Arrays.stream(c)).count());
    }
